  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        orderId: {
          type: 'string',
          description: 'Order number, e.g. ORD-4417.',
        },
      },
      required: ['orderId'],
      additionalProperties: false,
...
function executeTool(
  ...
  if (
    name === 'process_refund' &&
    Number(input['amountUsd']) > REFUND_CEILING_USD
  ) {
    return {
      isError: true,
      payload: {
  ...
  switch (name) {
    case 'get_customer': {
      ...
      if (matches.length === 0) {
        return {
          isError: false,
          payload: { found: false, reason: 'no_matches' },
        };
      }
      if (matches.length > 1) {
        return {
    ...
    case 'process_refund': {
      ...
      return {
        isError: false,
        payload: {
          refundId: `RFD-${order.orderId.slice(-4)}`,
          status: 'processed',
        },
      };
    }
...
async function main() {
  const log = startRunLog({ dir: import.meta.dir, label: 'manual-loop' });
  requireApiKey();
  resetTransientFailures();
  ...
  /**
   * Per-iteration stop reasons and token counts — the Task 1.1 evidence, and
   * the thing worth re-reading weeks later. Growth in `inputTokens` across
   * iterations is context accumulation made visible: each pass re-sends the
   * whole conversation, tool results included.
   */
  const perIteration: {
    n: number;
  ...
  while (true) {
    ...
    const iterationCalls: { name: string; input: unknown; isError: boolean }[] = [];
    perIteration.push({
      n: iterations,
    ...
    for (const use of toolUses) {
      ...
      console.log(
        `  → ${use.name}(${JSON.stringify(use.input)})` +
          (isError ? '  [error]' : ''),
      );
      iterationCalls.push({ name: use.name, input: use.input, isError });
      results.push({
        type: 'tool_result',
        tool_use_id: use.id,
  ...
  log.record({
    iterations,
    // The healthy value is "end_turn". Anything else means the loop stopped for
    // a reason other than the model deciding it was done.
    terminalStopReason: last?.stopReason ?? null,
    verifiedCustomerId: state.verifiedCustomerId,
    perIteration,
    // The raw Messages API reports tokens but not cost, and hand-rolling a
    // pricing table here would rot. Left null on purpose.
    costUsd: null,
  });
  log.close({ status: last?.stopReason === 'end_turn' ? 'ok' : 'failed' });
}
